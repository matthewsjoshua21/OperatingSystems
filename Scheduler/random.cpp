 ...
 */
int RandomNumGen::CPUBurstRandom(int mean) {
	//static long n = -10;
	//static long n = time(NULL);
	float x = poidev((float) mean, &seed);
	return (int) x;
}
 ...
 */
int RandomNumGen::ranInt(int min, int max) {
	//static long n = -1;
	float x = min + ran1() * (max - min);
	return (int) x;
}
 ...
 */
float RandomNumGen::gammln(float xx)
{
	double x,y,tmp,ser;
 ...
 */
float RandomNumGen::poidev(float xm, long *idum)
{
